...
class SmartTranslationManager:
    ...
    def setup_ui(self):
        ...
        self.file_path_var = tk.StringVar()
        ttk.Entry(file_inner, textvariable=self.file_path_var, width=50).grid(row=0, column=1, sticky="ew", padx=5)
        ttk.Button(file_inner, text="찾기", command=self.select_file).grid(row=0, column=2, padx=2)
        ttk.Button(file_inner, text="로드", command=self.load_data).grid(row=0, column=3, padx=2)
        
        file_inner.grid_columnconfigure(1, weight=1)
        
        # === 2. 번역 설정 & LLM 프롬프트 영역 (좌우 분할) ===
        ...
        ttk.Radiobutton(engine_frame, text="Azure", variable=self.api_engine_var, 
                    value="azure", command=self.on_engine_changed).pack(anchor="w")
        ttk.Radiobutton(engine_frame, text="LLM", variable=self.api_engine_var, 
                    value="llm", command=self.on_engine_changed).pack(anchor="w")
        ...
        ttk.Checkbutton(options_inner, text="EN", variable=self.translate_en_var).pack(anchor="w")
        ttk.Checkbutton(options_inner, text="다국어 (TH,PT,ES,FR,DE)", variable=self.translate_multi_var).pack(anchor="w")
        ttk.Checkbutton(options_inner, text="CN/TW", variable=self.translate_cn_tw_var).pack(anchor="w")
        ...
        ttk.Checkbutton(advanced_inner, text="특수 태그 보호", variable=self.protect_tags_var).pack(anchor="w")
        ttk.Checkbutton(advanced_inner, text="복잡한 마크업 처리", variable=self.complex_markup_var).pack(anchor="w")
        ttk.Checkbutton(advanced_inner, text="🎭 시나리오 번역 모드", variable=self.scenario_translation_var, 
                    command=self.on_scenario_option_changed).pack(anchor="w")
        
        # 우측: LLM 프롬프트 설정 영역 (더 넓게)
        self.llm_settings_frame = ttk.LabelFrame(settings_container, text="🤖 LLM 프롬프트 설정")
        self.llm_settings_frame.pack(side="right", fill="both", expand=True, padx=(5, 0))
        
        prompt_inner = ttk.Frame(self.llm_settings_frame, padding="10")
        ...
        ttk.Button(template_buttons_frame, text="게임", command=lambda: self.set_prompt_template("game")).pack(side="left", padx=1)
        ttk.Button(template_buttons_frame, text="일반", command=lambda: self.set_prompt_template("natural")).pack(side="left", padx=1)
        ttk.Button(template_buttons_frame, text="기술", command=lambda: self.set_prompt_template("technical")).pack(side="left", padx=1)
        ttk.Button(template_buttons_frame, text="캐주얼", command=lambda: self.set_prompt_template("casual")).pack(side="left", padx=1)
        ttk.Button(template_buttons_frame, text="초기화", command=lambda: self.set_prompt_template("default")).pack(side="left", padx=1)
        ...
        self.llm_prompt_entry = tk.Text(prompt_text_frame, height=8, wrap="word", font=("맑은 고딕", 9))
        ...
        ttk.Button(button_row1, text="🔍 번역 분석", command=self.analyze_translations).pack(side="left", padx=2)
        ttk.Button(button_row1, text="🚀 자동 번역", command=self.execute_translation).pack(side="left", padx=2)
        ttk.Button(button_row1, text="💾 결과 저장", command=self.save_results).pack(side="left", padx=2)
        
        # 상태 표시
        self.status_label = ttk.Label(button_row1, text="준비됨", foreground="blue")
        self.status_label.pack(side="right", padx=10)
        
        button_row2 = ttk.Frame(bottom_frame)
        button_row2.pack(fill="x", pady=2)
        
        ttk.Button(button_row2, text="🔄 선택 재번역", command=self.force_retranslate_selected).pack(side="left", padx=2)
        ttk.Button(button_row2, text="🗑️ 선택 TM삭제", command=self.remove_from_tm).pack(side="left", padx=2)
    ...
    def analyze_translations(self):
        ...
        for trans in self.pending_translations:
            ...
            if needs_api:
                ...
                api_needed_count += 1
                if kr_text in self.translation_memory:
                    print(f"TM에 있지만 API 필요: {kr_text} -> TM: {self.translation_memory[kr_text]}")
        ...
        self.update_translation_table()
        self.update_status(f"분석 완료. TM사용: {tm_used_count}, API필요: {api_needed_count}")
    ...
    # _force_retranslate_thread 아래에 추가할 새 함수
