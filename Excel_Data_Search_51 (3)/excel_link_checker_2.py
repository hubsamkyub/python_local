...
# -*- coding: utf-8 -*-
"""
엑셀 파일의 외부 링크 검사 도구 (GUI 포함) - 최종 완성 버전
"""
        ...
        try:
            ...
            try:
                if hasattr(workbook, 'external_links') and workbook.external_links:
                    print(f"🔗 워크북 외부 링크 발견: {workbook.external_links}")
                    ...
                    print(f"📝 워크북 외부 링크 없음")
            except Exception as e:
                print(f"❌ 워크북 외부 링크 검사 오류: {e}")
            
            # 방법 2: 명명된 범위 검사 - 디버깅 정보 추가
            try:
                if hasattr(workbook, 'defined_names') and workbook.defined_names:
                    keys = list(workbook.defined_names.keys())
                    print(f"📋 명명된 범위 발견: {len(keys)}개 - {keys}")
                    
                    for i, name_key in enumerate(keys):
                        print(f"  🔍 {i+1}. 검사 중: {name_key}")
                        try:
                            ...
                            if hasattr(defined_name, 'value') and defined_name.value:
                                ...
                                print(f"     값: '{name_formula}'")
                                
                                # #REF! 오류 우선 검사
                                ref_error_found = False
                                for j, ref_pattern in enumerate(self.ref_error_patterns):
                                    if re.search(ref_pattern, name_formula):
                                        print(f"     🚨 #REF! 패턴 {j+1} 매칭: {ref_pattern}")
                                        result['external_links'].append({
                                ...
                                if not ref_error_found:
                                    for j, pattern in enumerate(self.external_patterns):
                                        if re.search(pattern, name_formula):
                                            print(f"     🔗 외부 참조 패턴 {j+1} 매칭: {pattern}")
                                            result['external_links'].append({
                            ...
                            print(f"     ❌ 명명된 범위 '{name_key}' 처리 오류: {e}")
                else:
                    print(f"📝 명명된 범위 없음")
            except Exception as e:
                print(f"❌ 명명된 범위 검사 오류: {e}")
            
            # 방법 3: 셀별 외부 참조 검사 (제한적으로)
            print(f"🔍 셀 검사 시작 (최대 {max_cells}개)")
            cell_count = 0
            cell_issues_found = 0
                ...
                for row in worksheet.iter_rows():
                    for cell in row:
                        ...
                        if cell.data_type == 'f' and cell.value:
                            ...
                            for ref_pattern in self.ref_error_patterns:
                                if re.search(ref_pattern, formula):
                                    print(f"  🚨 셀 #REF! 발견: {sheet_name}!{cell.coordinate}")
                                    result['external_links'].append({
                                        'type': 'cell_ref_error',
                                ...
                                for pattern in self.external_patterns:
                                    if re.search(pattern, formula):
                                        print(f"  🔗 셀 외부 참조 발견: {sheet_name}!{cell.coordinate}")
                                        result['external_links'].append({
                                            'type': 'cell_external_link',
                                            'location': f"{sheet_name}!{cell.coordinate}",
                                            'formula': formula[:100] + ('...' if len(formula) > 100 else '')
                                        })
                                        cell_issues_found += 1
                        ...
                        # #REF! 값 검사
                            ...
                            print(f"  🚨 셀 #REF! 값 발견: {sheet_name}!{cell.coordinate}")
                            result['external_links'].append({
                                'type': 'cell_ref_value',
                                'location': f"{sheet_name}!{cell.coordinate}",
            ...
            print(f"📊 셀 검사 완료: {cell_count}개 셀 중 {cell_issues_found}개 문제 발견")
            
            workbook.close()
            
            # 최종 결과 확인
            total_issues = len(result['external_links'])
            print(f"🎯 총 {total_issues}개 문제 발견:")
            for i, item in enumerate(result['external_links'], 1):
                print(f"  {i}. {item['type']} - {item.get('name', item.get('location', 'Unknown'))}")
            
            # 외부 링크 발견 여부 설정
            result['has_external_links'] = total_issues > 0
            print(f"🏁 has_external_links = {result['has_external_links']}")
...
def test_patterns():
    ...
    for test_str in test_strings:
        ...
        print("외부 참조 패턴:")
        for i, pattern in enumerate(external_patterns):
