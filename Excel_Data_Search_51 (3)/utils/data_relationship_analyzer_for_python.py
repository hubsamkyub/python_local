...
class DatabaseRelationAnalyzer:
    ...
    def export_to_python_file(self, output_file: str = "db_relationships.py") -> bool:
        """분석 결과를 Python 파일로 내보내기"""
        if not self.relationships:
            print("분석 결과가 없습니다. analyze_all_relationships()를 먼저 실행해주세요.")
            return False
        
        try:
            with open(output_file, 'w', encoding='utf-8') as f:
                f.write('# 데이터베이스 관계 정보\n')
                f.write('# Database Relationship Information\n')
                f.write('# 자동 생성된 파일입니다. 수정하지 마세요.\n\n')
                ...
                # 관계 정보를 딕셔너리로 저장
                f.write('# 테이블 간 관계 정보\n')
                f.write('TABLE_RELATIONSHIPS = {\n')
                
                for rel in self.relationships:
                    key = f"{rel['table1']}.{rel['column1']}_to_{rel['table2']}.{rel['column2']}"
                    f.write(f'    "{key}": {{\n')
                    f.write(f'        "source_table": "{rel["table1"]}",\n')
                    f.write(f'        "source_column": "{rel["column1"]}",\n')
                    f.write(f'        "target_table": "{rel["table2"]}",\n')
                    f.write(f'        "target_column": "{rel["column2"]}",\n')
                    f.write(f'        "relationship_type": "{rel["relationship_type"]}",\n')
                    f.write(f'        "match_count": {rel["both_exist"]},\n')
                    f.write(f'        "match_ratio": {rel["match_ratio"]:.4f},\n')
                    f.write(f'        "source_only_count": {rel["table1_only"]},\n')
                    f.write(f'        "target_only_count": {rel["table2_only"]}\n')
                    f.write('    },\n')
                ...
                for rel in self.relationships:
                    table_relations[rel['table1']].append({
                        'type': 'outgoing',
                        'target_table': rel['table2'],
                        'source_column': rel['column1'],
                        'target_column': rel['column2'],
                        'relationship_type': rel['relationship_type'],
                        'match_count': rel['both_exist']
                    })
                    table_relations[rel['table2']].append({
                        'type': 'incoming',
...
''')
            ...
            print(f"✅ 관계 정보가 '{output_file}' 파일로 내보내졌습니다.")
            return True
...
def main():
    ...
    try:
        ...
        # 5. Python 파일로 내보내기
        print("💾 관계 정보를 Python 파일로 내보내는 중...")
        analyzer.export_to_python_file("db_relationships.py")
